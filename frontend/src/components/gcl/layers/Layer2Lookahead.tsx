...
import { gclApi } from '../../../api/client';
import type { LoopCycle, ScenarioMeta } from '../../../api/gcl-types';
...
export default function Layer2Lookahead() {
  ...
  const runScenario = useCallback(async () => {
    ...
    try {
      ...
      if (!scenarioSeeded) {
        meta = await gclApi.seedScenario('inference_fleet_spike', 42) as ScenarioMeta;
        seedScenario('inference_fleet_spike', 42, meta);
      }

      const distStep = (meta as any)?.disturbance_step ?? 4;
      ...
      for (let i = 0; i < TOTAL_STEPS; i++) {
        ...
        const scenarioStep = await gclApi.getStep(i);
        const response = await gclApi.runCycle(scenarioStep.signals);
        const full = await gclApi.getCycle(response.cycle_id);
        ...
        const latencySignals = scenarioStep.signals.filter(
          (s: { metric: string; value: number }) => s.metric === 'latency_ms',
        );
        const avgLatency =
          latencySignals.length > 0
            ? latencySignals.reduce((sum: number, s: { value: number }) => sum + s.value, 0) /
