...
import type { LoopCycle, ChainEntry } from '../../../api/gcl-types';
...
export default function Layer3Floor() {
  ...
  const runCycle = useCallback(async () => {
    ...
    try {
      if (!scenarioSeeded) {
        const meta = await gclApi.seedScenario('inference_fleet_spike', 42);
        seedScenario('inference_fleet_spike', 42, meta);
      }

      const step = await gclApi.getStep(1);
      const response = await gclApi.runCycle(step.signals);
      const [full, chainEntries] = await Promise.all([
        gclApi.getCycle(response.cycle_id),
        gclApi.getCycleChain(response.cycle_id),
      ]);
      setCycle(full);
    ...
    >
      ...
      {cycle && (
        ...
        >
          ...
          >
            It separates the creative framing from the governed execution
